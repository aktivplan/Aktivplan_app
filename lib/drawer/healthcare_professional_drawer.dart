...
class HealthcareProfessionalDrawer extends StatefulWidget {
  final Widget drawerHeader;
  HealthcareProfessionalDrawer({
    Key? key,
    required this.drawerHeader,
  }) : super(key: key);

  @override
  _HealthcareProfessionalDrawerState createState() => _HealthcareProfessionalDrawerState();
}

class _HealthcareProfessionalDrawerState extends State<HealthcareProfessionalDrawer> with LogoutAware {
  @override
    ...
    return Drawer(
      child: ListView(
        children: [
          widget.drawerHeader,
          ListTile(
              tileColor: RouterService.isDescendantActive(context, "/patients") ? primarySwatch[50] : Colors.transparent,
              leading: Icon(Icons.group, color: primaryColor),
              title: Text(
                context.i18n.patientOverview,
                style: Theme.of(context).textTheme.titleSmall?.copyWith(
                      color: RouterService.isDescendantActive(context, "/patients") ? Theme.of(context).primaryColor : lightTextColor,
                      fontWeight: FontWeight.w600,
                    ),
              ),
              onTap: () async {
                BlocProvider.of<ActivityBloc>(context).add(ResetActivityEvent());
                context.beamToNamed("/patients", data: {
                  'healthcareProfessionalId': userRepository.currentUser.id,
                  'institutionId': userRepository.currentUser.institutionId,
                });
              }),
          ListTile(
          ...
          if (!(userRepository.currentInstitution!.institutionFocus?.isKlimafit() ?? false))
            ListTile(
