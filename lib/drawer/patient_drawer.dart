...
class _PatientDrawerState extends State<PatientDrawer> with LogoutAware {
  bool _isConnectToGoogleHealthConnectAppleHealth = false;

  @override
  void initState() {
    super.initState();
    BlocProvider.of<SensorBloc>(context)..add(IsConnectToGoogleHealthConnectAppleHealthEvent());
  }
        ...
        children: [
          ...
          if (!kIsWeb)
            BlocBuilder<SensorBloc, SensorState>(builder: (context, state) {
              String title = "";
              if (state is IsConnectToGoogleHealthConnectAppleHealthState) {
                _isConnectToGoogleHealthConnectAppleHealth = state.isConnected;
              }
              if (Platform.isIOS)
                title = _isConnectToGoogleHealthConnectAppleHealth ? context.i18n.disconnectFromAppleHealth : context.i18n.connectToAppleHealth;
              else
                title = _isConnectToGoogleHealthConnectAppleHealth
                    ? context.i18n.disconnectFromGoogleHealthConnect
                    : context.i18n.connectToGoogleHealthConnect;
              return ListTile(
                  tileColor: Colors.transparent,
                  leading: Icon(Icons.health_and_safety_sharp, color: lightTextColor),
                  title: Text(
                    title,
                    style: Theme.of(context).textTheme.titleSmall?.copyWith(
                          color: lightTextColor,
                  ...
                  onTap: () async => {
                        if (!_isConnectToGoogleHealthConnectAppleHealth)
                          BlocProvider.of<SensorBloc>(context).add(ConnectToGoogleHealthConnectAppleHealthEvent(healthDataTypeList: [
                            HealthDataType.HEART_RATE,
                            HealthDataType.STEPS,
                            HealthDataType.WORKOUT,
                          ]))
                        else
                          BlocProvider.of<SensorBloc>(context).add(DisconnectToGoogleHealthConnectAppleHealthEvent()),
