...
class TasksTable extends StatefulWidget {
  ...
  @override
  _TasksTableState createState() => _TasksTableState();
}
    ...
    return SingleChildScrollView(
      child: ResponsiveBuilder(
        builder: (context, size) {
          ...
          if (widget.fetchedTasks.isEmpty)
            return Container(
              height: height * 0.55,
              child: Center(
                child: Padding(
            ...
            return DataTable(
              ...
              columns: [
                ...
                if (!size.isMobile)
                  AptDataColumn(
                    label: Text(context.i18n.youTubeUrl),
                  ),
                AptDataColumn(
                  label: Text(""),
                )
              ],
              rows: widget.fetchedTasks
                  .map(
                    (task) => DataRow(
                      onSelectChanged: (selected) => widget.editExercise(task),
                      cells: [
                        DataCell(
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                if (getTranslatedText(task.hint, context).isEmpty)
                                  IconButton(
                                    icon: Icon(Icons.info_outline, color: Colors.black12),
                                    onPressed: null,
                                  ),
                                if (getTranslatedText(task.hint, context).isNotEmpty)
                                  Tooltip(
                                    padding: EdgeInsets.all(8.0),
                                    preferBelow: true,
                                    message: getTranslatedText(task.hint, context),
                                ...
                                Flexible(
                                  child: Container(
                                    child: Wrap(
                                      children: [
                                        SelectableText(getTranslatedText(task.name, context)),
                                      ],
                                    ),
                                  ),
                                ),
                        ...
                        if (!size.isMobile)
                          DataCell(
                            Container(
                              ...
                              child: Text(
                                getTranslatedText(task.youTubeUrl, context),
                                style: Theme.of(context).textTheme.bodyLarge?.copyWith(
                                      letterSpacing: 1.1,
                                    ),
                              ),
                            ...
                            child: Row(
                              ...
                              children: [
                                IconButton(
                                  ...
                                  onPressed: () => widget.editExercise(task),
                                ),
                              ],
                            ),
                          ),
