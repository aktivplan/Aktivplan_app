// Copyright LBI-DHP and/or licensed to LBI-DHP under one or more
// contributor license agreements (LBI-DHP: Ludwig Boltzmann Institute
// for Digital Health and Prevention -- A research institute of the
// Ludwig Boltzmann Gesellschaft, Österreichische Vereinigung zur
// Förderung der wissenschaftlichen Forschung).
// Licensed under the Apache 2.0 license with Commons Clause
// (see https://www.apache.org/licenses/LICENSE-2.0 and
// https://commonsclause.com/).

//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:apt_api/api.dart';
import 'package:test/test.dart';

// tests for DatahubResponseRecommendationsInner
void main() {
  // final instance = DatahubResponseRecommendationsInner();

  group('test DatahubResponseRecommendationsInner', () {
    // RecommendationType type
    test('to test the property `type`', () async {
      // TODO
    });

    // RecommendationLevel level
    test('to test the property `level`', () async {
      // TODO
    });

    // RecoomendationReason reason
    test('to test the property `reason`', () async {
      // TODO
    });

    // String reasonDetail
    test('to test the property `reasonDetail`', () async {
      // TODO
    });

    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // ActivityType activityType
    test('to test the property `activityType`', () async {
      // TODO
    });

    // String activity
    test('to test the property `activity`', () async {
      // TODO
    });

    // String action
    test('to test the property `action`', () async {
      // TODO
    });

    // DateTime timeFrom
    test('to test the property `timeFrom`', () async {
      // TODO
    });

    // DateTime timeUntil
    test('to test the property `timeUntil`', () async {
      // TODO
    });

    // String originActivityID
    test('to test the property `originActivityID`', () async {
      // TODO
    });

    // String destinationActivityID
    test('to test the property `destinationActivityID`', () async {
      // TODO
    });

    // LocationDTO origin
    test('to test the property `origin`', () async {
      // TODO
    });

    // LocationDTO destination
    test('to test the property `destination`', () async {
      // TODO
    });

    // MobilityPreference mode
    test('to test the property `mode`', () async {
      // TODO
    });

    // String route
    test('to test the property `route`', () async {
      // TODO
    });

    // int duration
    test('to test the property `duration`', () async {
      // TODO
    });

    // int elevation
    test('to test the property `elevation`', () async {
      // TODO
    });

    // SpaceTimeRecommendationChangeType changeType
    test('to test the property `changeType`', () async {
      // TODO
    });
  });
}
