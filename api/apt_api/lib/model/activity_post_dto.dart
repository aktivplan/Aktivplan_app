...
class ActivityPostDTO {
  /// Returns a new [ActivityPostDTO] instance.
  ActivityPostDTO({
    this.plannedBy,
    this.patientId,
    this.startDate,
  ...
  /// Returns a new [ActivityPostDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ActivityPostDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "ActivityPostDTO[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "ActivityPostDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return ActivityPostDTO(
  ...
  static List<ActivityPostDTO> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <ActivityPostDTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ActivityPostDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ActivityPostDTO> mapFromJson(dynamic json) {
    final map = <String, ActivityPostDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ActivityPostDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ActivityPostDTO-objects as value to a dart map
  static Map<String, List<ActivityPostDTO>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<ActivityPostDTO>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ActivityPostDTO.listFromJson(
          entry.value,
          growable: growable,
        );
      }
