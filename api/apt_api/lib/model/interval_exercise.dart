...
class IntervalExercise {
  /// Returns a new [IntervalExercise] instance.
  IntervalExercise({
    this.id,
    this.institutionId,
    this.importId,
  ...
  /// Returns a new [IntervalExercise] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static IntervalExercise? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "IntervalExercise[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "IntervalExercise[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<IntervalExercise> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <IntervalExercise>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = IntervalExercise.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, IntervalExercise> mapFromJson(dynamic json) {
    final map = <String, IntervalExercise>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = IntervalExercise.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of IntervalExercise-objects as value to a dart map
  static Map<String, List<IntervalExercise>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<IntervalExercise>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = IntervalExercise.listFromJson(
          entry.value,
          growable: growable,
        );
      }
