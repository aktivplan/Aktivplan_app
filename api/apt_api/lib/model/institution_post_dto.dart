...
class InstitutionPostDTO {
  /// Returns a new [InstitutionPostDTO] instance.
  InstitutionPostDTO({
    this.name,
  ...
  @override
  String toString() =>
      'InstitutionPostDTO[name=$name, institutionFocus=$institutionFocus, url=$url, email=$email, firstName=$firstName, lastName=$lastName, showTrainingPlans=$showTrainingPlans, allowRescheduleActivities=$allowRescheduleActivities, enableSocialFeatures=$enableSocialFeatures, emailUserQueries=$emailUserQueries, phoneNumberUserQueries=$phoneNumberUserQueries, availabilityPhone=$availabilityPhone, getInTouchNotes=$getInTouchNotes]';
  ...
  /// Returns a new [InstitutionPostDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static InstitutionPostDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "InstitutionPostDTO[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "InstitutionPostDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<InstitutionPostDTO> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <InstitutionPostDTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = InstitutionPostDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, InstitutionPostDTO> mapFromJson(dynamic json) {
    final map = <String, InstitutionPostDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = InstitutionPostDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of InstitutionPostDTO-objects as value to a dart map
  static Map<String, List<InstitutionPostDTO>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<InstitutionPostDTO>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = InstitutionPostDTO.listFromJson(
          entry.value,
          growable: growable,
        );
      }
