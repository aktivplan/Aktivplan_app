...
class Recommendation {
  ...
  @override
  String toString() =>
      'Recommendation[type=$type, level=$level, reason=$reason, reasonDetail=$reasonDetail, description=$description]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Recommendation[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Recommendation[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, List<Recommendation>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Recommendation.listFromJson(
          entry.value,
          growable: growable,
        );
      }
