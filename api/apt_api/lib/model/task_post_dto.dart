...
class TaskPostDTO {
  /// Returns a new [TaskPostDTO] instance.
  TaskPostDTO({
    this.hint = const {},
    this.name = const {},
  ...
  @override
  String toString() =>
      'TaskPostDTO[hint=$hint, name=$name, youTubeUrl=$youTubeUrl, videoFileKey=$videoFileKey, type=$type, exerciseIntensityPercentageStart=$exerciseIntensityPercentageStart, exerciseIntensityPercentageEnd=$exerciseIntensityPercentageEnd, exerciseTrainingHeartRateLowerLimit=$exerciseTrainingHeartRateLowerLimit, exerciseTrainingHeartRateUpperLimit=$exerciseTrainingHeartRateUpperLimit, exerciseDurationSeconds=$exerciseDurationSeconds]';
  ...
  /// Returns a new [TaskPostDTO] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static TaskPostDTO? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "TaskPostDTO[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "TaskPostDTO[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<TaskPostDTO> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <TaskPostDTO>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TaskPostDTO.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TaskPostDTO> mapFromJson(dynamic json) {
    final map = <String, TaskPostDTO>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TaskPostDTO.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of TaskPostDTO-objects as value to a dart map
  static Map<String, List<TaskPostDTO>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<TaskPostDTO>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = TaskPostDTO.listFromJson(
          entry.value,
          growable: growable,
        );
      }
